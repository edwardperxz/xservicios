<?php
$this->assign('header-title', 'Detalle de Configuración');
?>
...
<div class="view-container">
    <div class="view-card">
        <div class="view-header">
            <h2 class="view-title">Configuración: <?= h($xservConfiguracione->clave) ?></h2>
            <div class="view-actions">
                <a href="<?= $this->Url->build(['action' => 'edit', $xservConfiguracione->id]) ?>" class="btn btn-primary">Editar</a>
                <a href="<?= $this->Url->build(['action' => 'index']) ?>" class="btn btn-secondary">Volver al Listado</a>
                <?= $this->Form->postLink('Eliminar', ['action' => 'delete', $xservConfiguracione->id], ['confirm' => '¿Está seguro?', 'class' => 'btn btn-danger']) ?>
            </div>
        </div>

        <div class="detail-grid">
            <div class="detail-item">
                <div class="detail-label">Clave</div>
                <div class="detail-value"><?= h($xservConfiguracione->clave) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Tipo Dato</div>
                <div class="detail-value"><?= h($xservConfiguracione->tipo_dato) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Grupo</div>
                <div class="detail-value"><?= h($xservConfiguracione->grupo) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Editable Por Admin</div>
                <div class="detail-value"><?= $xservConfiguracione->editable_por_admin ? 'Sí' : 'No' ?></div>
            </div>
            <div class="detail-item" style="grid-column: 1 / -1;">
                <div class="detail-label">Descripción Parámetro</div>
                <div class="detail-value"><?= h($xservConfiguracione->descripcion_parametro) ?></div>
            </div>
            <div class="detail-item" style="grid-column: 1 / -1;">
                <div class="detail-label">Valor</div>
                <div class="detail-value"><?= h($xservConfiguracione->valor) ?></div>
            </div>
        </div>
    </div>
</div>