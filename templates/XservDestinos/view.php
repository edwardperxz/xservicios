<?php
$this->assign('header-title', 'Detalle de Destino');
?>
...
<div class="view-container">
    <div class="view-card">
        <div class="view-header">
            <h2 class="view-title">Destino: <?= $xservDestino->hasValue('ubicacion') ? h($xservDestino->ubicacion->nombre) : 'ID #' . $xservDestino->id ?></h2>
            <div class="view-actions">
                <a href="<?= $this->Url->build(['action' => 'edit', $xservDestino->id]) ?>" class="btn btn-primary">Editar</a>
                <a href="<?= $this->Url->build(['action' => 'index']) ?>" class="btn btn-secondary">Volver al Listado</a>
                <?= $this->Form->postLink('Eliminar', ['action' => 'delete', $xservDestino->id], ['confirm' => '¿Está seguro?', 'class' => 'btn btn-danger']) ?>
            </div>
        </div>
        ...
        <div class="detail-grid">
            <div class="detail-item">
                ...
                <div class="detail-value"><?= $xservDestino->hasValue('ubicacion') ? $this->Html->link($xservDestino->ubicacion->nombre, ['controller' => 'XservUbicaciones', 'action' => 'view', $xservDestino->ubicacion->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Es Popular</div>
                <div class="detail-value"><?= $xservDestino->es_popular ? 'Sí' : 'No' ?></div>
            </div>
            <div class="detail-item" style="grid-column: 1 / -1;">
                <div class="detail-label">Descripción (Español)</div>
                <div class="detail-value"><?= h($xservDestino->descripcion_es) ?></div>
            </div>
            <div class="detail-item" style="grid-column: 1 / -1;">
                <div class="detail-label">Descripción (Inglés)</div>
                <div class="detail-value"><?= h($xservDestino->descripcion_en) ?></div>
            </div>
        </div>
    </div>
</div>