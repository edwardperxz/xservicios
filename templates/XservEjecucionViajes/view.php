<?php
$this->assign('header-title', 'Detalle de Ejecución de Viaje');
?>
...
<div class="view-container">
    <div class="view-card">
        <div class="view-header">
            <h2 class="view-title">Ejecución de Viaje #<?= h($xservEjecucionViaje->id) ?></h2>
            <div class="view-actions">
                <a href="<?= $this->Url->build(['action' => 'edit', $xservEjecucionViaje->id]) ?>" class="btn btn-primary">Editar</a>
                <a href="<?= $this->Url->build(['action' => 'index']) ?>" class="btn btn-secondary">Volver al Listado</a>
                <?= $this->Form->postLink('Eliminar', ['action' => 'delete', $xservEjecucionViaje->id], ['confirm' => '¿Está seguro?', 'class' => 'btn btn-danger']) ?>
            </div>
        </div>
        ...
        <div class="detail-grid">
            <div class="detail-item">
                ...
                <div class="detail-value"><?= $xservEjecucionViaje->hasValue('asignacion') ? $this->Html->link('Asignación #' . $xservEjecucionViaje->asignacion->id, ['controller' => 'XservAsignaciones', 'action' => 'view', $xservEjecucionViaje->asignacion->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Estado Ejecución</div>
                <div class="detail-value"><?= h($xservEjecucionViaje->estado_ejecucion) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">KM Inicio</div>
                <div class="detail-value"><?= $xservEjecucionViaje->km_inicio === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->km_inicio) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">KM Fin</div>
                <div class="detail-value"><?= $xservEjecucionViaje->km_fin === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->km_fin) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Latitud Inicio</div>
                <div class="detail-value"><?= $xservEjecucionViaje->lat_inicio === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->lat_inicio) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Longitud Inicio</div>
                <div class="detail-value"><?= $xservEjecucionViaje->lng_inicio === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->lng_inicio) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Latitud Fin</div>
                <div class="detail-value"><?= $xservEjecucionViaje->lat_fin === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->lat_fin) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Longitud Fin</div>
                <div class="detail-value"><?= $xservEjecucionViaje->lng_fin === null ? 'N/A' : $this->Number->format($xservEjecucionViaje->lng_fin) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Hora Inicio Real</div>
                <div class="detail-value"><?= h($xservEjecucionViaje->hora_inicio_real) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Hora Fin Real</div>
                <div class="detail-value"><?= h($xservEjecucionViaje->hora_fin_real) ?></div>
            </div>
            <div class="detail-item" style="grid-column: 1 / -1;">
                <div class="detail-label">Observaciones Finales</div>
                <div class="detail-value"><?= h($xservEjecucionViaje->observaciones_finales) ?></div>
            </div>
        </div>
    </div>
</div>