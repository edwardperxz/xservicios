<?php
$this->assign('header-title', 'Detalle de Ruta');
?>
...
<div class="view-container">
    <div class="view-card">
        <div class="view-header">
            <h2 class="view-title">Ruta #<?= h($xservRuta->id) ?></h2>
            <div class="view-actions">
                <a href="<?= $this->Url->build(['action' => 'edit', $xservRuta->id]) ?>" class="btn btn-primary">Editar</a>
                <a href="<?= $this->Url->build(['action' => 'index']) ?>" class="btn btn-secondary">Volver al Listado</a>
                <?= $this->Form->postLink('Eliminar', ['action' => 'delete', $xservRuta->id], ['confirm' => '¿Está seguro?', 'class' => 'btn btn-danger']) ?>
            </div>
        </div>
        ...
        <div class="detail-grid">
            <div class="detail-item">
                ...
                <div class="detail-value"><?= $xservRuta->hasValue('origen') ? $this->Html->link($xservRuta->origen->nombre, ['controller' => 'XservUbicaciones', 'action' => 'view', $xservRuta->origen->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Destino</div>
                <div class="detail-value"><?= $xservRuta->hasValue('destino') ? $this->Html->link($xservRuta->destino->nombre, ['controller' => 'XservUbicaciones', 'action' => 'view', $xservRuta->destino->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Precio Base</div>
                <div class="detail-value">$<?= $this->Number->format($xservRuta->precio_base) ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Tiempo Estimado (min)</div>
                <div class="detail-value"><?= $xservRuta->tiempo_estimado_min === null ? 'N/A' : $this->Number->format($xservRuta->tiempo_estimado_min) . ' min' ?></div>
            </div>
        </div>
    </div>
</div>