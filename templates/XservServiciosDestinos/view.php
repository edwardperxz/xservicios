<?php
$this->assign('header-title', 'Detalle de Servicio-Destino');
?>
...
<div class="view-container">
    <div class="view-card">
        <div class="view-header">
            <h2 class="view-title">Servicio-Destino</h2>
            <div class="view-actions">
                <a href="<?= $this->Url->build(['action' => 'edit', $xservServiciosDestino->servicio_id]) ?>" class="btn btn-primary">Editar</a>
                <a href="<?= $this->Url->build(['action' => 'index']) ?>" class="btn btn-secondary">Volver al Listado</a>
                <?= $this->Form->postLink('Eliminar', ['action' => 'delete', $xservServiciosDestino->servicio_id], ['confirm' => '¿Está seguro?', 'class' => 'btn btn-danger']) ?>
            </div>
        </div>
        ...
        <div class="detail-grid">
            <div class="detail-item">
                ...
                <div class="detail-value"><?= $xservServiciosDestino->hasValue('servicio') ? $this->Html->link($xservServiciosDestino->servicio->nombre, ['controller' => 'XservServicios', 'action' => 'view', $xservServiciosDestino->servicio->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Destino</div>
                <div class="detail-value"><?= $xservServiciosDestino->hasValue('destino') ? $this->Html->link('Destino #' . $xservServiciosDestino->destino->id, ['controller' => 'XservDestinos', 'action' => 'view', $xservServiciosDestino->destino->id]) : '<span style="color: #6b7280;">No asignado</span>' ?></div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Orden Visita</div>
                <div class="detail-value"><?= $xservServiciosDestino->orden_visita === null ? 'N/A' : $this->Number->format($xservServiciosDestino->orden_visita) ?></div>
            </div>
        </div>
    </div>
</div>